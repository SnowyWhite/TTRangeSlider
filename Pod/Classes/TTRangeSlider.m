...
- (void)initialiseControl {
    ...
    _handleDiameter = 24.0;
    _selectedHandleDiameterMultiplier = 1.7;
    
    _lineHeight = 1.0;
    _lineHeightBetweenHandles = 2.0;
    _handleColor = [UIColor whiteColor];
    
    _handleBorderColor = [UIColor colorWithRed:148.0/255 green:66.0/255 blue:89.0/255 alpha:1];
    _handleBorderWidth = 1;
    
    //draw the slider line
    self.sliderLine = [CALayer layer];
    self.sliderLine.backgroundColor = [UIColor colorWithWhite:225.0/255 alpha:1].CGColor;  // self.tintColor.CGColor;
...
- (void)updateHandlePositions {
    ...
    self.sliderLineBetweenHandles.frame = CGRectMake(self.leftHandle.position.x, self.sliderLine.frame.origin.y, self.rightHandle.position.x-self.leftHandle.position.x, self.lineHeightBetweenHandles);
}
...
-(void)setLineHeightBetweenHandles:(CGFloat)lineHeightBetweenHandles{
    _lineHeightBetweenHandles = lineHeightBetweenHandles;
    [self setNeedsLayout];
}
